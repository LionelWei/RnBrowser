...
- (void)playWithUrl:(NSURL *)url showView:(UIView *)showView
{
    ...
    if (IOS_VERSION < 7.0 || ![str hasPrefix:@"http"]||[str hasPrefix:@"http://127.0.0.1"]) {
    ...
    self.currentPlayerLayer.backgroundColor = [UIColor blackColor].CGColor;
    if (!_videoView) {
      self.videoView = [[UIView alloc]initWithFrame:showView.bounds];
      [showView addSubview:_videoView];
    }
    [showView bringSubviewToFront:_videoView];
    [_videoView.layer addSublayer:self.currentPlayerLayer];
    ...
    if (!_closeButton) {
        ...
        [_closeButton setFrame:CGRectMake(kScreenWidth - 60, 20, 44, 44)];
        [_videoView addSubview:_closeButton];
    }
    [_videoView bringSubviewToFront:_closeButton];
  
    if (!_navBar) {
        _navBar = [[UIView alloc] init];
        [_videoView addSubview:_navBar];
    }
    [_videoView bringSubviewToFront:_navBar];
    [self buildVideoNavBar];
    
    [[XCHudHelper sharedInstance] showHudOnView:_showView caption:nil image:nil acitivity:YES autoHideTime:30];
...
- (void)closeButtonAction:(id)sender{
    //cancel connection
    [[NSNotificationCenter defaultCenter] postNotificationName:@"DisconnectedRequest" object:nil];
    [self stop];
    _playSlider.value = 0;
  ...
  _currentTimeLabel = nil;
    [_totolTimeLabel removeFromSuperview];
  _totolTimeLabel = nil;
    [_videoProgressView removeFromSuperview];
  _videoProgressView = nil;
...
{
    _navBar.backgroundColor = [UIColor blackColor];//[self colorWithHex:0x000000 alpha:0.5];
    _navBar.frame = CGRectMake(0, kScreenHeight - 44, kScreenWidth, 44);
